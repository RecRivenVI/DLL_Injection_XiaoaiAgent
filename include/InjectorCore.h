...
// 从注册表获取安装路径
inline std::string GetExecutablePathFromRegistry(const std::string &subKey, const std::string &valueName)
{
    HKEY hKey;
...
inline int RunInjector(const std::string &consoleTitle, const std::string &regSubKey, const std::string &exeName, const std::string &errRegMsg)
{
    // 设置控制台标题
    ...
    {
        MessageBoxA(NULL, errRegMsg.c_str(), "注入失败", MB_ICONERROR);
        return 1;
    }
    ...
    if (!WriteProcessMemory(pi.hProcess, pRemoteMem, (LPVOID)dllAbsolutePath.c_str(), dllAbsolutePath.length() + 1, NULL))
        goto cleanup;
